...
// Reduced raymarching from 39 → 24 iterations — still looks identical as a
// background but costs ~40% less GPU per pixel.
const fragment = `
...
vec2 sceneC(vec2 frag, vec2 r) {
  vec2 P=(frag+frag-r)/r.x; float z=0.0,d=1e3;
  vec4 O=vec4(0.0);
  // 24 iterations (was 39) — good enough for a fullscreen background
  for(int k=0;k<24;k++){
    if(d<=1e-4) break;
...
void mainImage(out vec4 o, vec2 C) {
  vec2 r=iResolution.xy, uv0=(C+C-r)/r.x;
  ...
  vec2 P2=vec2(2,1)*uv0-(r/r.x)*vec2(0,1);
  ...
  float zr=5e-4*uStreakWidth, tail=19.0/max(uStreakLength,0.05);
  vec2 rr=vec2(max(length(fw),1e-5));
  for(int m=0;m<16;m++){
    if(m>=uStreakCount) break;
    float jf=float(m)+1.0;
    float ic=fract(sin(dot(vec2(jf,floor(C.x/Y.x+0.5)),vec2(7,11))*73.0));
    vec2 Pp=C-(T+T*ic)*vec2(0,1); Pp-=floor(Pp/Y+0.5)*Y;
...
export default function Lightfall({
  ...
  useEffect(() => {
    ...
    // Cap DPR at 1 — this is a background shader, not UI chrome.
    // Retina rendering buys nothing visually but costs 4× the pixel work.
    const effectiveDpr = dpr ?? 1;

    const renderer = new Renderer({ dpr: effectiveDpr, alpha: true, antialias: false });
    ...
    const onPointerMove = (e: PointerEvent) => {
      ...
      const scale = effectiveDpr;
      mouseTarget.current = [
        (e.clientX - rect.left) * scale,
        (rect.height - (e.clientY - rect.top)) * scale,
      ];
      if (mouseDampening <= 0) uniforms.iMouse.value = [...mouseTarget.current];
    };
    if (mouseInteraction) canvas.addEventListener("pointermove", onPointerMove, { passive: true });

    // Stop rendering when the canvas is scrolled out of view
    let isVisible = true;
    ...
    const loop = (t: number) => {
      ...
      if (!isVisible || document.hidden || contextLost || paused) return;
    ...
    return () => {
      ...
      programRef.current = geometryRef.current = meshRef.current = rendererRef.current = null;
    };
